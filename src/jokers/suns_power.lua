            ...
            if PlayedHand ~= 'NULL' then
                ...
                if PlayedHand == G.GAME.current_round.cstorm_suns_power_hand then
                    update_hand_text({ sound = 'button', volume = 0.7, pitch = 0.8, delay = 0.3 },
                        {
                            handname = localize(G.GAME.current_round.cstorm_suns_power_hand, 'poker_hands'),
                            chips = G.GAME.hands
                                [G.GAME.current_round.cstorm_suns_power_hand].chips,
                            mult = G.GAME.hands
                                [G.GAME.current_round.cstorm_suns_power_hand].l_mult,
                            level = G.GAME.hands
