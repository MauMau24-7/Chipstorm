...
extern PRECISION vec2 goldShader;
...
vec4 goldShader_color = vec4(231., 164., 25., 0.) / 255.;
...
bool line(vec2 uv, float offset, float width) {
    ...
    offset = offset + 0.35 * sin(goldShader.x + TWO_PI * lines_offset);
    width = width + 0.005 * sin(goldShader.x);
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    pixel = vec4(goldShader_color.rgb * avg + tex.rgb * tex.a, pixel.a);
