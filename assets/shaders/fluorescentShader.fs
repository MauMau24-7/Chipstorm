...
extern PRECISION vec2 fluorescentShader;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float t = fluorescentShader.g + time;
    float a = 0.93;
    float b = 1.1;
    float c = 1.3;

    if (fluorescentShader.g > 0.0 || fluorescentShader.g < 0.0) {
        ...
        if (hsl.y < 0.2) {
            ...
            hsl.y *= 2 * (b*cos(a*t) + c);  
            float res = (.01 + .01* cos( (fluorescentShader.r)*4 ));
            hsl.x += res;

            // hsl.y *= cos(fluorescentShader.r*0.513);
